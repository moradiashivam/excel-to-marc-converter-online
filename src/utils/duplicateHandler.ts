...
/**
 * Finds duplicate records in the data based on matching 020$a (ISBN) and 250$a (Edition) fields
...
export const findDuplicateRecords = (data: MarcData[]): Array<{firstIndex: number, secondIndex: number}> => {
  ...
  const isbnEditionMap = new Map<string, number>();
  
  // First pass: build a map of ISBN + Edition combinations to their first occurrence index
  data.forEach((record, index) => {
    const isbn = record['020$a'];
    const edition = record['250$a'];
    
    // Only process records that have both ISBN and Edition
    if (isbn && edition) {
      const key = `${isbn}__${edition}`;
      if (isbnEditionMap.has(key)) {
        // Found a duplicate, record its index
        duplicates.push({
          firstIndex: isbnEditionMap.get(key)!,
          secondIndex: index
        });
      } else {
        // Record the first occurrence
        isbnEditionMap.set(key, index);
      }
    }
  });
